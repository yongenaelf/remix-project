...
import { ExampleContract } from '../components/Result'
import EventEmitter from 'events'
import { Plugin } from "@remixproject/engine";
import { CustomRemixApi } from '@remix-api'

export type VyperComplierAddress = 'https://playground-next.test.aelf.dev/' | 'http://localhost:8000/'
export class RemixClient extends PluginClient<any, CustomRemixApi> {
  ...
  async cloneRepo(count?: number) {

    try {
      // @ts-ignore
      this.call('notification', 'toast', 'cloning Examples repository...')     
      await this.call(
        'dgitApi',
        'clone',
        {url: 'https://github.com/AElfProject/aelf-samples', token: null, branch: 'master', singleBranch: false, workspaceName: 'Examples'},
      )
      ...
      this.call(
        ...
        'notification',
        'toast',
        'Examples repository cloned, the workspace Examples has been created.'
      )
    } catch (e) {
      // @ts-ignore
