...
if (domains[window.location.hostname]) {
  ...
  if (!window.localStorage.getItem('config-v0.8:.remix.config') || (window.localStorage.getItem('config-v0.8:.remix.config') && !window.localStorage.getItem('config-v0.8:.remix.config').includes('settings/matomo-analytics'))) {
...
fetch(versionUrl, {cache: 'no-store'}).then((response) => {
  response.text().then(function (data) {
    ...
    console.log(`Loading aelf Studio ${version.version}`)
  })
})
